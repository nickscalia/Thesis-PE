...
sys.path.append('../../../lib')
from emg_utils import bandpass_filter,rectification, RMS_moving, MVC_normalization, extract_emg_features
...
def main():    
    # External Variables    
    mvc = '../../../data/stream/mvc/test_2.csv'
    model = load('../../../data/stream/models/GB/gb_emg.pkl')
    scaler_new = load('../../../data/stream/models/GB/gb_emg_scaler.pkl')    
    ...
    """
    imu_scalers = load('./../../data/stream/imu_scalers/test_1.pkl)
    fs_imu = 50 
    ...
    streamer, shared_memory = myo_streamer(emg=True, imu=False, filtered=False)
            ...
            if sample_counter > window_feature_size:
                ...
                if len(all_predictions) >= 4:
                    ...
                    if (all(c == last_classes[0] for c in last_classes) and
                        all(p > 0.7 for p in last_probas)):
