...
class EMGApp:
    def __init__(self, master):
        ...
        # All'inizio del tuo __init__
        style = Style()

        # Crea uno stile personalizzato per i bottoni
    ...
    def normalization(self, signal, muscle_name, norm_values):
        """
            ...
            while not self.interrupt_flag:
                ...
                if 'emg' in data and len(data['emg']) > 0:
                    ...
                    for ch in range(8):
                        ...
                        emg_rectified = rectification(emg_filtered)
                        emg_rms = RMS_moving(emg_rectified, self.fs)
                        emg_norm_filtered = self.normalization(emg_rms, self.channel_names[ch], self.norm_values)
                    ...
                    if len(all_predictions) >= 3:
                        ...
                        if start_transition is not None: 
                            ...
                            if elapsed > 0.5:
                                ...
                                filtered_classes = [cls for cls, prob in zip(recent_classes, recent_probas) if prob >0.5]
                            ...
                            elif (all(c == last_classes[0] for c in last_classes) and
