...
with NumpySocket() as s:
    ...
    for depth_map,frame_resize in run_depth_sensor():
                ...
                if start_time - last_send_time < min_frame_time:
                        continue
                ...
                #frame_resize = np.rot90(frame_resize)
                ...
                #data2 = np.array(gray)
                #_,encoded = cv2.imencode('.jpg', gray, [cv2.IMWRITE_JPEG_QUALITY, 60])
                                
                try:
                    s.sendall(data) # send the images via socket cloud
                    last_send_time = time.time()
                    #s.sendall(encoded) # send the images via socket cloud
                    #s.sendall(data2) # send the images via socket cloud
