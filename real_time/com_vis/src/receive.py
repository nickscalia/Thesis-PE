...
# This code uses socket to create a bridge between the raspberry pi and the mac, and receive the images acquired by the raspberry pi 
# to the local pc (mac) for processing. 

# N.B.: You will need a camera connected to the mackbook to run this code.

# This script uses the environment "PipelineEvironment.yml"

# %% START THE SCRIPT
print('----------------------------------------- INITIALIZATION -----------------------------------------')
#%% IMPRORT THE NECESSARY LIBRARIES & DEFINE USEFUL FUCNTIONS
...
def ObjectDetectionAndIsolation(frame):
    ...
    # Detect objects
    results1 = model(frame, stream=True, device=device, imgsz=800)
    ...
    #conf_thresholds_model = {'box': 0.50}
    ...
    if np.array(final_probabilities).any(): # if at least one object was detected in the environment, do the rest
        ...
        pd.DataFrame({'a':[PositionsAndDimensions[which][4]], 'b':[PositionsAndDimensions[which][5]], 'c':[PositionsAndDimensions[which][6]], 'd':[PositionsAndDimensions[which][7]]}).to_csv('data.csv')
        # save object image (for later payload estimation)
        cv2.imwrite("candidate.png", candidate_image)
        ...
        if PositionsAndDimensions[which][6] > 0.1:
            pd.DataFrame({'boolean':[1]}).to_csv('gate.csv')
        else:
            pd.DataFrame({'boolean':[0]}).to_csv('gate.csv')
...
print('----> YOLO-v8 model general set-up')
# YOLO model 8 set-up weights 
color = (0, 65, 255)
#device = torch.device('mps' if torch.backends.mps.is_available() else 'cpu')
device = torch.device('xpu' if torch.xpu.is_available() else 'cpu')
model = YOLO("yolo-Weights/yolov11_new.pt")
print(device)

print('---- CLASSES NAMES ----')
print('The following classes are the ones contained in the YOLO-v8 fine-tuned model:\n', model.names)
...
with NumpySocket() as s:
    ...
    cap.set(cv2.CAP_PROP_FPS, 10)  # ad esempio 30 FPS
    
    # Controlla se l'impostazione ha avuto effetto
    fps = cap.get(cv2.CAP_PROP_FPS)
    print(f"FPS effettivi: {fps}")
    """
        
    if not cap.isOpened():
        print("Errore nell'apertura del flusso")
        exit()
    width = cap.get(cv2.CAP_PROP_FRAME_WIDTH)
    height = cap.get(cv2.CAP_PROP_FRAME_HEIGHT)
    print(f"Risoluzione corrente: {int(width)} x {int(height)}") 
        ...
        try:
            ...
            while conn: # if and until the connection is true (which means you are connected over the cloud) do
                if last_frame is None:
                    continue
                iphone_frame = last_frame.copy()
                #ret, iphone_frame = cap.read() # take an image form the phone camera (it couold be any general remote camera)
                #print(iphone_frame.shape, iphone_frame.dtype)
                #print(np.allclose(iphone_frame[:,:,0], iphone_frame[:,:,1]))
                #print(np.allclose(iphone_frame[:,:,1], iphone_frame[:,:,2]))
                #if not ret: # if you didn't manage to colelct the image break  
                    #print('Error: issues encountered while trying to collect an image. Check embedded camera connection...')
                    #break
                iphone_frame = cv2.cvtColor(iphone_frame,cv2.COLOR_RGB2GRAY) # ocnvert to gray for more standard and general processing 
                #iphone_frame = cv2.resize(iphone_frame, (int(iphone_frame.shape[1]/2),int(iphone_frame.shape[0]/2))) # reduce image size if high-quality camera 
                ...
                print("Packet 1 received: ", frame.nbytes)

                #frame2 = conn.recv() # the data you receive here are sent from raspberry pi in np.uint8. If you transfer data in np.uint8 it's hundreds of times faster
                #print("Packet 2 received: ", frame2.nbytes)
                #frame2 = cv2.imdecode(frame2, cv2.IMREAD_GRAYSCALE)
                frame2 = iphone_frame
                ...
                #confid_buf = frame2[:,0:width_image] # divide confidence buffer from the depth map
                ...
                #sensor_depth = frame[:,width_image:-1] 
                #frame = np.concatenate((confid_buf, sensor_depth), axis=1) # concatenate image and received depth map
                #print(frame2.shape[1],frame2.shape[0])

                frame = np.concatenate((frame2, cv2.resize(sensor_depth, (frame2.shape[1],frame2.shape[0]))), axis=1) # concatenate image and received depth map
                ...
                final_class = ObjectDetectionAndIsolation(frame) # process the data
